  ...
  useEffect(() => {
    const setupAudio = async () => {
      try {
        ...
        Alert.alert("Setup Error", "Failed to initialize audio");
      }
    };
    setupAudio();
  ...
  const exportConversation = async () => {
    try {
      if (conversation.length === 0) {
        Alert.alert("No Conversation", "There are no messages to export");
        return;
      }
      ...
      const conversationText = conversation
        .map((msg) => {
          ...
          return `[${time}] ${msg.isUser ? "You" : "Companion"}: ${msg.text}`;
        })
        .join("\n\n");
      ...
      if (await Sharing.isAvailableAsync()) {
        ...
        Alert.alert("Export Complete", "Conversation saved to device");
      }
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Your AI friend to talk with</Text>
          ...
          <View style={styles.emptyState}>
            <Text style={styles.emptyStateText}>
              {currentLanguage === "english"
                ? "Tap the microphone to start talking with your companion"
                : "點擊麥克風開始與您的伴侶交談"}
            </Text>
            <TouchableOpacity
