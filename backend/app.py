...
DEEPSEEK_MODEL = os.getenv("DEEPSEEK_MODEL", "llama3.1:latest")
...
def generate_audio_azure(text, language='english'):
    try:
        ...
        # Check if result is not None before accessing attributes
        if result is None:
            print("❌ TTS failed: No result returned")
            return None
...
print("Initializing activities service...")
activities_service = ActivitiesService()
print("Activities service initialized successfully")
...
def filter_activities():
    try:
        ...
        activity_type = data.get('activityType', 'All Types')
        user_location = data.get('userLocation')
        start_mtr_station = data.get('startMtrStation')
        
        if not district:
            return jsonify({'error': 'District is required'}), 400
        
        # Filter activities
        filtered_activities = activities_service.filter_activities(district, activity_type)
        
        # Enhance activities with MTR directions
        if start_mtr_station:
            # Use the selected MTR station
            enhanced_activities = activities_service.enhance_activities_with_directions(
                filtered_activities, start_mtr_station
            )
        else:
            # Use location-based detection or fallback
            location = activities_service.get_user_location(user_location)
            if location:
                # Find nearest MTR station to the user location
                nearest_station = activities_service.find_nearest_mtr_station(location)
                if nearest_station:
                    enhanced_activities = activities_service.enhance_activities_with_directions(
                        filtered_activities, nearest_station["name"]
                    )
                else:
                    enhanced_activities = filtered_activities
            else:
                enhanced_activities = filtered_activities
        ...
        print(f"Error filtering activities: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/activities/recommend', methods=['POST'])
def recommend_activities():
    try:
        data = request.json
        print(f"=== recommend_activities called ===")
        ...
        print(f"District: {district}, Activity Type: {activity_type}")
        ...
        filtered_activities = activities_service.filter_activities(district, activity_type)
        print(f"Filtered activities count: {len(filtered_activities)}")
        
        # Get GenAI recommendations
        print("Getting GenAI recommendations...")
        recommended_activities = activities_service.get_genai_recommendations(
            {**user_preferences, 'district': district, 'activity_type': activity_type}, 
            filtered_activities
        )
        print(f"Recommended activities count: {len(recommended_activities)}")
        
        # Enhance activities with MTR directions
        print("Enhancing activities with MTR directions...")
        if start_mtr_station:
            # Use the selected MTR station
            enhanced_activities = activities_service.enhance_activities_with_directions(
                recommended_activities, start_mtr_station
            )
        else:
            # Use location-based detection or fallback
            location = activities_service.get_user_location(user_location)
            if location:
                ...
                if nearest_station:
                    enhanced_activities = activities_service.enhance_activities_with_directions(
                        recommended_activities, nearest_station["name"]
                    )
                else:
                    enhanced_activities = recommended_activities
            else:
                enhanced_activities = recommended_activities
        
        print(f"Enhanced activities count: {len(enhanced_activities)}")
        print(f"=== recommend_activities completed ===")
        return jsonify({
            'activities': enhanced_activities,
            'count': len(enhanced_activities)
        }), 200
        ...
        print(f"Error recommending activities: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500

@app.route('/activities/summary', methods=['POST'])
def generate_activity_summary():
    try:
        ...
        if language == 'cantonese':
            prompt = f"""請以廣東話為以下活動產生一個簡單易明的摘要，專為長者而設：
            
            活動名稱: {activity.get('name', '')}
            類別: {activity.get('category', '')}
            地點: {activity.get('venue', '')}
            日期: {activity.get('date', '')}
            {f"時間: {activity.get('time', '')}" if activity.get('time') else ''}
            費用: {activity.get('fee', '')}
            描述: {activity.get('description', '')}
            
            請提供一個簡潔易懂的摘要，重點介紹長者需要知道的關鍵資訊。請只用廣東話回覆。"""
        else:
            prompt = f"""Please generate a simple and clear summary for the following activity, designed for elderly users:
            
            Activity Name: {activity.get('name', '')}
            Category: {activity.get('category', '')}
            Venue: {activity.get('venue', '')}
            Date: {activity.get('date', '')}
            {f"Time: {activity.get('time', '')}" if activity.get('time') else ''}
            Fee: {activity.get('fee', '')}
            Description: {activity.get('description', '')}
            
            Please provide a concise and easy-to-understand summary that highlights the key information an elderly person would need to know."""
        
        # Generate summary using Ollama
        try:
            response = activities_service.ollama_client.chat(
            ...
            if hasattr(response, 'message') and hasattr(response.message, 'content'):
                summary = response.message.content.strip() if response.message.content else ""
            else:
            ...
            print(f"Error calling Ollama: {ollama_error}")
            # Fallback to a simple summary if Ollama fails
            fallback_summary = f"{activity.get('name', '')} is a {activity.get('category', '')} activity at {activity.get('venue', '')} on {activity.get('date', '')}."
            if language == 'cantonese':
                fallback_summary = f"「{activity.get('name', '')}」係一個{activity.get('category', '')}活動，喺{activity.get('venue', '')}舉行，日期係{activity.get('date', '')}。"
            return jsonify({
